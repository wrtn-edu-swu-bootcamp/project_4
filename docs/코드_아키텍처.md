...
graph TB
    ...
    Storage -->|용어 데이터| IndexedDB[(IndexedDB)]
...
| 기술 | 용도 | 특징 |
|------|------|------|
| **IndexedDB** | 용어 사전 저장 | - 30개+ 용어 저장<br>- 빠른 검색 (인덱싱)<br>- 오프라인 동작 |
...
├──────────────────────────┤
│      Data Layer          │  ← Storage Utils (IndexedDB, Chrome Storage)
└──────────────────────────┘
```
...
### 1. 용어 데이터 스키마 (IndexedDB)

```javascript
// Object Store: "terms"
{
  id: "term-101",                    // 고유 ID (Primary Key)
  name: "복리",                       // 용어명 (인덱스)
  aliases: ["복리 이자", "복리계산"],  // 별칭 배열
  category: "금리 관련",              // 카테고리
  description: "이자에 이자가...",    // 간단 설명 (1-2문장)
  example: "100만원 연 3%...",        // 구체적 예시 (숫자 포함)
  importance: "복리는...",            // 왜 중요한가
  relatedTerms: ["단리", "연이율"],   // 관련 용어 (최대 3개)
  createdAt: "2026-01-22",           // 생성일
  updatedAt: "2026-01-22"            // 수정일
}
```

### 2. 사용자 설정 스키마 (Chrome Storage)
...
// chrome.storage.sync
{
  settings: {
    ...
    enableAnalytics: false,           // 사용 통계 (현재 미사용)
...
chrome.runtime.onInstalled.addListener(async (details) => {
  if (details.reason === 'install') {
    // 첫 설치: IndexedDB 초기화 + 온보딩
    await initDB();
    await loadInitialTerms();
...
```javascript
async function handleTextSelection(selectedText, position) {
  // 1. 텍스트 정규화
  const normalized = normalizeText(selectedText);
  
  // 2. 용어 검색
  const term = await findTerm(normalized);
...
async function getCachedTerms() {
  ...
  cachedTerms = await getAllTerms(); // IndexedDB에서 로드
  cacheTime = Date.now();
  return cachedTerms;
}
```
...
async function initDB() {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open('FinancialTermsDB', 1);
...
sequenceDiagram
    ...
    ServiceWorker->>IndexedDB: 데이터베이스 생성
    ServiceWorker->>IndexedDB: 용어 데이터 로드
    IndexedDB-->>ServiceWorker: 완료
...
sequenceDiagram
    ...
    TermMatcher->>IndexedDB: 용어 검색
    IndexedDB-->>TermMatcher: 용어 데이터 또는 null
    ...
    alt 매칭 성공
        ...
        User->>PopupRenderer: 외부 검색 클릭
        PopupRenderer->>Chrome: 새 탭으로 검색
    end
...
sequenceDiagram
    ...
    ChromeStorage-->>Popup: enabled, termCount 등
    Popup->>User: ON/OFF 상태 & 통계 표시
...
async function getCachedTerms() {
  if (cachedTerms && Date.now() - cacheTime < CACHE_TTL) {
    return cachedTerms; // 캐시 히트
  }
  
  // 캐시 미스: IndexedDB에서 로드
  cachedTerms = await getAllTerms();
  cacheTime = Date.now();
  return cachedTerms;
}
```
...
**용어 데이터**:
- 확장 설치 시: JSON → IndexedDB 마이그레이션
- 이후: IndexedDB에서 직접 로드 (파일 I/O 제거)
...
| 용어 검색 (캐시 히트) | ~1ms | < 5ms |
| 용어 검색 (캐시 미스) | ~50ms | < 100ms |
| IndexedDB 초기화 | ~200ms | < 500ms |
