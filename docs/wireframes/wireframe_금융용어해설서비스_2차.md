# 금융 용어 해설 서비스 - Wireframe 문서 (2차)

> **서비스명**: 금융 용어 쉽게 (가칭)  
> **문서 버전**: v2.0  
> **최종 수정일**: 2026-01-22  
> **작성자**: judy0  
> **주요 변경**: 사용자 선택 기반 인터랙션 모델로 전환
...
3. [핵심 기능 화면](#3-핵심-기능-화면)
   - 3.1 [은행 웹사이트 - 용어 선택 및 조회](#31-은행-웹사이트---용어-선택-및-조회)
...
전체 서비스의 사용자 경험 플로우입니다. (2차: 사용자 선택 기반 모델)
...
graph TD
    ...
    BankVisit --> ReadTerms[약관 페이지 읽기]
    
    ReadTerms --> UserAction{이해 안 되는 단어?}
    UserAction -->|없음| Continue[약관 계속 읽기]
    UserAction -->|있음| SelectWord[단어 드래그 또는 더블클릭]
    
    SelectWord --> CheckTerm[선택된 텍스트 확인]
    CheckTerm --> TermExists{용어 사전에 있는가?}
    
    TermExists -->|예| ShowPopup[즉시 팝업 표시]
    TermExists -->|아니오| NoMatch[일치하는 용어 없음]
    
    NoMatch --> Continue
    ShowPopup --> PopupView[용어 설명 확인]
    ...
    Settings --> SettingsMenu[ON/OFF 및 설정 변경]
    SettingsMenu --> ReadTerms
```
...
graph TD
    ...
    Layout1 --> Logo[서비스 로고 - 금융 용어 쉽게]
    Layout1 --> Title1[환영합니다!]
    Layout1 --> Subtitle1[약관 읽기가 쉬워집니다]
    Layout1 --> MainMessage[금융 상품의 약관과 상품 설명을 읽을 때 - 이해 안 되는 단어를 선택하면 - 즉시 쉬운 설명을 확인할 수 있습니다]
    
    Layout1 --> Icon1[아이콘 1: 마우스 클릭]
    Layout1 --> Icon2[아이콘 2: 전구]
    Layout1 --> Icon3[아이콘 3: 체크]
    
    Icon1 --> Text1[단어 선택]
    Icon2 --> Text2[즉시 설명 표시]
    Icon3 --> Text3[빠른 이해]
...
graph TD
    ...
    StepSection --> Step2[Step 2: 이해 안 되는 단어 선택]
    StepSection --> Step3[Step 3: 팝업에서 설명 확인]
    
    Step1 --> Step1Img[일러스트: 은행 사이트 화면]
    Step1 --> Step1Text[KB국민은행, 신한은행 등 - 어떤 금융기관이든 작동]
    
    Step2 --> Step2Img[일러스트: 단어 선택]
    Step2 --> Step2Text[복리, 세전금리 등 - 드래그 또는 더블클릭으로 선택]
    
    Step3 --> Step3Img[일러스트: 팝업 표시]
    Step3 --> Step3Text[쉬운 설명과 예시를 - 선택 즉시 확인]
    
    Layout2 --> TipBox[💡 팁: 툴바 아이콘을 클릭하면 - 언제든 ON/OFF 가능]
...
graph TD
    ...
    ValidCheck -->|예| Success[온보딩 완료 - 자동으로 탭 닫힘]
...
### 3.1 은행 웹사이트 - 용어 선택 및 조회

사용자가 은행 웹사이트에서 단어를 선택했을 때 확장 프로그램이 작동하는 화면입니다.
...
graph TD
    ...
    PageLoad --> ExtReady[확장 프로그램 대기 상태]
    
    ExtReady --> UserReading[사용자가 약관 읽기]
    UserReading --> UserSelect{단어 선택?}
    
    UserSelect -->|드래그| DragSelect[텍스트 드래그]
    UserSelect -->|더블클릭| DoubleClick[단어 더블클릭]
    UserSelect -->|없음| ContinueRead[계속 읽기]
    
    DragSelect --> CaptureText[선택된 텍스트 캡처]
    DoubleClick --> CaptureText
    
    CaptureText --> Normalize[텍스트 정규화 - 공백 제거, 소문자 변환]
    Normalize --> SearchDict[용어 사전에서 검색]
    
    SearchDict --> Found{일치하는 용어?}
    Found -->|예| ShowPopup[즉시 팝업 표시]
    Found -->|아니오| NoResult[검색 결과 없음 - 아무 동작 없음]
    
    ShowPopup --> PopupPosition[팝업 위치 계산]
    PopupPosition --> DisplayPopup[선택 영역 근처에 팝업 표시]
    
    NoResult --> ContinueRead
    
    BankPage --> ExampleLayout[화면 예시 구조]
    
    ExampleLayout --> BankHeader[은행 헤더 - KB국민은행 로고]
    ExampleLayout --> ProductTitle[상품명: KB 정기예금]
    ExampleLayout --> ProductDesc[상품 설명 영역]
    
    ProductDesc --> Text1[연이율 3.5%]
    ProductDesc --> Text2[복리 적용 - 사용자 선택 가능]
    ProductDesc --> Text3[세전금리 기준 - 사용자 선택 가능]
    ProductDesc --> Text4[만기일시지급식 - 사용자 선택 가능]
    ProductDesc --> Text5[예금자보호법 적용 - 사용자 선택 가능]
    
    Text2 --> SelectAction[드래그/더블클릭 시]
    Text3 --> SelectAction
    Text4 --> SelectAction
    Text5 --> SelectAction
    
    SelectAction --> PopupTrigger[팝업 트리거]
```

### 3.2 용어 팝업 - PC 버전

PC 환경에서 단어를 선택했을 때 표시되는 팝업 구조입니다.
...
graph TD
    ...
    Trigger --> UserSelect[단어 드래그 또는 더블클릭]
    UserSelect --> InstantShow[즉시 팝업 표시]
    
    InstantShow --> PopupLayout[팝업 레이아웃 구조]
    ...
    Section1 --> Desc[이자에도 이자가 붙는 방식이에요 - 마치 눈덩이가 굴러가면서 - 점점 커지는 것처럼...]
    
    Section2 --> ExIcon[💡 예시]
    Section2 --> ExText[100만원을 연 3% 복리로 2년간 - 예금하면... - 1년 후: 103만원 - 2년 후: 106만900원]
    
    Section3 --> WhyIcon[❗ 왜 중요한가요?]
    Section3 --> WhyText[같은 금리라도 복리로 계산하면 - 단리보다 수익이 많아요]
    ...
    Footer --> Disclaimer[※ 본 설명은 참고용이며 - 정확한 정보는 해당 금융기관에 - 확인하세요.]
    
    PopupLayout --> Position[팝업 위치]
    Position --> NearSelection[선택 영역 근처]
    Position --> Above[선택 영역 위쪽 우선]
    ...
    CloseAction -->|다른 텍스트 선택| NewSelection[새 팝업 표시]
```
...
graph TD
    ...
    MobilePopup --> MTrigger[트리거: 텍스트 선택]
    MTrigger --> MSelect[롱프레스 또는 드래그로 선택]
    MSelect --> MShow[즉시 팝업 표시]
...
graph TD
    ...
    Panel1 --> G1[서비스 활성화]
    G1 --> G1Toggle[ON/OFF 스위치 - 기본: ON]
    G1 --> G1Desc[금융 사이트 방문 시 - 용어 조회 기능 활성화]
    
    Panel1 --> G2[지원 사이트]
    G2 --> G2List[모든 금융기관 사이트 - 특정 사이트만 선택 옵션]
    
    Panel1 --> G3[선택 방식]
    G3 --> G3Options[선택 옵션]
    G3Options --> G3Opt1[⚪ 드래그 + 더블클릭 기본]
    G3Options --> G3Opt2[⚪ 드래그만]
    G3Options --> G3Opt3[⚪ 더블클릭만]
    
    Panel2 --> D1[팝업 위치]
    D1 --> D1Options[선택 옵션]
    D1Options --> D1Opt1[⚪ 선택 영역 위쪽]
    D1Options --> D1Opt2[⚪ 선택 영역 아래쪽]
    D1Options --> D1Opt3[⚪ 자동 조정 기본]
    
    Panel2 --> D2[팝업 크기]
    D2 --> D2Slider[슬라이더: 소-중-대 - 기본: 중]
    
    Panel2 --> D3[팝업 투명도]
    D3 --> D3Slider[슬라이더: 80% ~ 100% - 기본: 95%]
    
    Panel2 --> PreviewBox[미리보기 영역]
    PreviewBox --> PreviewText[복리 팝업 예시]
    
    Panel3 --> P1[데이터 수집]
    P1 --> P1Checkbox[☐ 익명 사용 통계 수집 동의]
    P1 --> P1Desc[서비스 개선 목적으로만 사용 - 개인정보는 수집하지 않습니다]
    
    Panel3 --> P2[로컬 데이터]
    P2 --> P2Info[비교 메모: 3건 - 설정 정보: 로컬 저장]
    P2 --> P2Btn[모두 삭제 버튼]
    ...
    Panel4 --> I1[서비스 정보]
    I1 --> I1Version[버전: 2.0.0]
    I1 --> I1Date[최종 업데이트: 2026-01-22]
    I1 --> I1Model[인터랙션: 사용자 선택 기반]
    ...
    Panel4 --> I4[법적 고지]
    I4 --> I4Text[본 서비스는 참고용 정보 제공 - 투자 자문 아님]
```
...
graph TD
    ...
    Item1 --> ITime[작성: 5분 전]
    Item1 --> IContent[메모 내용: - 금리 3.5%, 복리 적용 - 우대조건: 급여이체]
...
graph TD
    ...
    Pr1 --> Pr1Detail[최소한의 정보만 수집 - 설정 정보 로컬 저장]
    ...
    Dis1 --> Dis1Text[참고용 정보이며 - 법적 효력 없음]
    Dis2 --> Dis2Text[서비스 정보 활용으로 인한 - 손해 책임 없음]
```
...
sequenceDiagram
    ...
    Bank->>User: 상품 페이지 로드
    Ext->>Ext: 대기 상태 (자동 동작 없음)
    
    User->>User: 약관 읽기 시작
    User->>Bank: "복리" 단어 드래그 선택
    Ext->>Ext: 선택된 텍스트 캡처
    Ext->>Ext: 용어 사전 검색
    Ext->>User: 즉시 팝업 표시
...
stateDiagram-v2
    ...
    ReadingTerms --> UnknownWord: 이해 안 되는 단어 발견
    
    UnknownWord --> DragWord: 단어 드래그
    UnknownWord --> DoubleClick: 단어 더블클릭
    
    DragWord --> CheckDict: 용어 사전 확인
    DoubleClick --> CheckDict
    
    CheckDict --> TermFound: 용어 사전에 존재
    CheckDict --> NoMatch: 일치하는 용어 없음
    
    NoMatch --> ReadingTerms: 약관 계속 읽기
    
    TermFound --> PopupShow: 즉시 팝업 표시
    ...
    ReadingTerms --> NextWord: 다음 단어 발견
    NextWord --> UnknownWord
...
graph TD
    ...
    Memo1 -->|예| WriteMemo1[메모 작성: - KB - 금리 3.5%, 복리]
    ...
    Memo2 -->|예| WriteMemo2[메모 작성: - 신한 - 금리 3.3%, 단리]
    ...
    CompareMemos --> Analysis[메모 목록 확인 - KB vs 신한 vs 하나]
...
```mermaid
graph TD
    SelectionDetect[텍스트 선택 감지]
    
    SelectionDetect --> EventListener[이벤트 리스너]
    EventListener --> MouseUp[mouseup 이벤트]
    EventListener --> SelectionChange[selectionchange 이벤트]
    EventListener --> DblClick[dblclick 이벤트]
    
    MouseUp --> GetSelection[window.getSelection]
    SelectionChange --> GetSelection
    DblClick --> GetSelection
    
    GetSelection --> ValidateSelection{유효한 선택?}
    ValidateSelection -->|예| ProcessText[텍스트 처리]
    ValidateSelection -->|아니오| Ignore[무시]
    
    ProcessText --> Normalize[정규화]
    Normalize --> Trim[공백 제거]
    Normalize --> LowerCase[소문자 변환]
    Normalize --> RemoveSpecial[특수문자 제거]
    
    RemoveSpecial --> SearchDict[용어 사전 검색]
    SearchDict --> DisplayPopup[팝업 표시]
```
...
graph LR
    ...
    Desktop --> DSelection[드래그 + 더블클릭]
    Desktop --> DKeyboard[키보드 단축키]
    
    Tablet --> TLayout[팝업: 가변 너비 60%]
    Tablet --> TSelection[드래그 + 터치]
    
    Mobile --> MLayout[팝업: 전체 너비 90%]
    Mobile --> MSelection[롱프레스로 선택]
...
graph TD
    ...
    Performance --> Passive[수동적 접근]
    Performance --> Efficient[효율적 검색]
    Performance --> Memory[메모리 관리]
    
    Passive --> NoAutoScan[자동 스캔 없음]
    Passive --> OnDemand[사용자 요청 시에만]
    Passive --> LowImpact[페이지 로드 영향 최소]
    
    Efficient --> QuickLookup[빠른 검색 알고리즘]
    Efficient --> IndexedDict[인덱스된 용어 사전]
    Efficient --> Cache[검색 결과 캐싱]
    
    Memory --> Lightweight[가벼운 확장]
    Memory --> SinglePopup[동시 팝업 1개만]
...
| 버전 | 작성일 | 주요 변경사항 |
|------|--------|--------------|
| v1.0 | 2026-01-22 | 최초 작성 - 자동 하이라이트 방식 |
| v2.0 | 2026-01-22 | 사용자 선택 기반 인터랙션 모델로 전환<br>- 자동 스캔/하이라이트 제거<br>- 드래그/더블클릭 선택 방식 도입<br>- 즉시 팝업 표시 방식으로 변경<br>- 설정 항목 재구성 |
